...
export default function AboutPage() {
    ...
    <div style={{ minHeight: '100vh', background: '#FDF6EC', paddingTop: 80 }}>
            {/* Hero */}
            <section style={{ background: 'linear-gradient(135deg, #1A0E06 0%, #3A1F0A 50%, #2C1A0E 100%)', padding: '80px 0 64px', overflow: 'hidden', position: 'relative' }}>
                {/* dot texture */}
                <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(circle, rgba(201,125,14,0.05) 1px, transparent 1px)', backgroundSize: '28px 28px', pointerEvents: 'none' }} />
                {/* Amber glow top-right */}
                <div style={{ position: 'absolute', top: -80, right: -40, width: 360, height: 360, borderRadius: '50%', background: 'radial-gradient(circle, rgba(201,125,14,0.1) 0%, transparent 70%)', pointerEvents: 'none' }} />
                    <div className="container about-hero-grid">
                    <div>
                        <div className="section-label" style={{ color: '#C97D0E' }}>Our Story</div>
                        <h1 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 'clamp(2.2rem, 4.5vw, 3.25rem)', color: '#F5E6C8', fontWeight: 800, marginBottom: 18, letterSpacing: '-0.025em' }}>
                            {displayStory.heading}
                        </h1>
                        <p style={{ color: 'rgba(245,230,200,0.75)', fontSize: 16, lineHeight: 1.8, marginBottom: 24 }}>
                            {displayStory.storyText}
                        </p>
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 28 }}>
                            {['Birds', 'Exotic Cats', 'Rodents', 'Reptiles', 'Turtles'].map(tag => (
                                <span key={tag} style={{ padding: '5px 14px', borderRadius: 6, background: 'rgba(201,125,14,0.1)', color: '#E8A020', fontSize: 11, fontWeight: 700, border: '1px solid rgba(201,125,14,0.2)', letterSpacing: '1px', textTransform: 'uppercase' }}>{tag}</span>
                            ))}
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                            ...
                            <div key={s.label} style={{ background: 'rgba(245,230,200,0.06)', borderRadius: 14, padding: '22px 20px', textAlign: 'center', border: '1px solid rgba(245,230,200,0.08)' }}>
                                <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 28, fontWeight: 800, color: '#C97D0E', lineHeight: 1 }}>{s.value}</div>
                                <div style={{ fontSize: 12, color: 'rgba(245,230,200,0.55)', marginTop: 6, letterSpacing: '0.4px' }}>{s.label}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section style={{ padding: '72px 0', background: '#fff' }}>
                <div className="container" style={{ textAlign: 'center', maxWidth: 760 }}>
                    ...
                    <p style={{ color: '#6B3A2A', fontSize: 16, lineHeight: 1.85, marginBottom: 44 }}>
                    ...
                    <div className="three-col-responsive">
                        {[
                            { icon: <Shield size={22} />, title: 'Ethical Sourcing', desc: 'All animals are captive-bred from licensed breeders — never wild-caught.' },
                            { icon: <Heart size={22} />, title: 'Lifelong Support', desc: 'We stay with you beyond the purchase — diet guides, vet referrals, and more.' },
                            { icon: <Award size={22} />, title: 'Quality First', desc: '100% health-certified pets with a 15-day guarantee on every animal.' },
                        ].map(v => (
                            <div key={v.title} style={{ padding: '24px 20px', background: '#FDF6EC', borderRadius: 14, textAlign: 'left', border: '1px solid rgba(44,26,14,0.05)' }}>
                                <div style={{ width: 48, height: 48, borderRadius: 12, background: 'rgba(201,125,14,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#C97D0E', marginBottom: 14 }}>{v.icon}</div>
                                <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 16, fontWeight: 700, color: '#2C1A0E', marginBottom: 6 }}>{v.title}</h3>
                                <p style={{ fontSize: 13.5, color: '#6B3A2A', lineHeight: 1.7 }}>{v.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="container">
                    ...
                    <div className="grid-4">
                        {team.map(m => {
                            ...
                            return (
                            <div key={m.name} style={{ background: '#fff', borderRadius: 16, padding: '24px 20px', textAlign: 'center', boxShadow: '0 2px 12px rgba(44,26,14,0.06)', transition: 'all 0.25s', border: '1px solid rgba(44,26,14,0.05)' }}
                                ...
                                onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 2px 12px rgba(44,26,14,0.06)'; }}>
                                <div style={{ width: 72, height: 72, borderRadius: '50%', background: colors[idx % colors.length], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 800, color: '#fff', margin: '0 auto 14px', fontFamily: "'Outfit', sans-serif", boxShadow: '0 4px 14px rgba(44,26,14,0.15)' }}>{initials}</div>
                                <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 16, fontWeight: 700, color: '#2C1A0E', marginBottom: 3 }}>{m.name}</h3>
                                <p style={{ fontSize: 11.5, color: '#C97D0E', fontWeight: 600, letterSpacing: 0.5, marginBottom: 10, textTransform: 'uppercase' }}>{m.role}</p>
                                <p style={{ fontSize: 13, color: '#6B3A2A', lineHeight: 1.65 }}>{m.bio}</p>
                            </div>
                        )})}
                    </div>
                </div>
