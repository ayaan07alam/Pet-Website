...
async function main() {
    ...
    for (const cat of mockCategories) {
        if (cat.id !== 'accessories' && cat.id !== 'food') {
    ...
    // 4. Seed Homepage Stats — only if table is empty
    ...
    // 5. Seed Animal Facts — only if table is empty
    const factsCount = await prisma.animalFact.count();
    if (factsCount === 0) {
        await prisma.animalFact.createMany({
            data: [
    ...
    // 6. Seed Why Us Features — only if table is empty
    const featuresCount = await prisma.whyUsFeature.count();
    if (featuresCount === 0) {
    ...
    // 7. Seed Testimonials — only if table is empty
